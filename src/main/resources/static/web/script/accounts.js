let app = new Vue({
  ...
  methods: {
    ...
    deleteAccount(number) {
      axios
        .patch("/api/accounts/", `number=${number}&bool=false`)
        .then((response) => {
    ...
    transfer() {
      axios
        .post(
          "/api/transactions",
          `amount=${this.accountDelete.balance}&description="transferencia por baja"&numberAccount=${this.accountDelete.number}&numberAccountDestiny=${this.cuentaDestino}`,
          { headers: { "content-type": "application/x-www-form-urlencoded" } }
        )
        .then((response) => {
          console.log("tranf");
